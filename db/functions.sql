...
CREATE OR REPLACE FUNCTION insert_user(user_id INTEGER, first_name VARCHAR(50), last_name VARCHAR(50), username VARCHAR(50)) RETURNS VOID AS $$
  BEGIN
...
CREATE OR REPLACE FUNCTION insert_user_in_chat(_chat_id INTEGER, _user_id INTEGER, _since DATE, _is_admin BOOl) RETURNS VOID AS $$
  DECLARE
  ...
  BEGIN
    ...
    IF (_chat_id, local_user_id) IN (SELECT chat_id, user_id FROM users_in_chats) THEN
      UPDATE users_in_chats SET (since, is_admin) = (_since, _is_admin)
      WHERE chat_id = _chat_id AND user_id = local_user_id;
    ELSE
      INSERT INTO users_in_chats VALUES (_chat_id, local_user_id, _since, _is_admin);
    END IF;
...
CREATE OR REPLACE FUNCTION insert_message(_msg_id INTEGER, _msg_text VARCHAR(4000), _date VARCHAR(50), _chat_id INTEGER, _user_id INTEGER) RETURNS VOID AS $$
  DECLARE
    local_user_id INTEGER := (SELECT local_id FROM users WHERE tg_id = _user_id);
  BEGIN
    ...
    IF char_length(msg_text) > 500 THEN
